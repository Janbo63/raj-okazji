    ...
    <div className="space-y-16">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            { name: 'Tools & DIY', pl: 'Narzędzia', icon: <Hammer className="w-8 h-8" />, color: 'bg-orange-100 text-orange-600' },
            { name: 'Home & Kitchen', pl: 'Dom i Kuchnia', icon: <HomeIcon className="w-8 h-8" />, color: 'bg-blue-100 text-blue-600' },
            { name: 'Electronics', pl: 'Elektronika', icon: <Cpu className="w-8 h-8" />, color: 'bg-brand-100 text-brand-600' },
            { name: 'Garden', pl: 'Ogród', icon: <Leaf className="w-8 h-8" />, color: 'bg-green-100 text-green-600' },
            { name: 'Clothes', pl: 'Odzież', icon: <Shirt className="w-8 h-8" />, color: 'bg-pink-100 text-pink-600' }
          ].map(cat => (
            <Link
              key={cat.name}
            ...
            {aiResponse && (
              <div className="mt-8 p-6 bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 animate-in fade-in slide-in-from-top-4">
                <p className="text-sm font-bold text-brand-200 mb-1 flex items-center gap-2 uppercase tracking-widest">
                  <Bot className="w-4 h-4" /> {TRANSLATIONS.aiAssistantLabel[lang]}:
                </p>
